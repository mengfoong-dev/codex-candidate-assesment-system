...
This repeatable, no-network validation has two layers:
...
| Provider outage or absent key | The endpoint emits `llm_unavailable`, persists a technical-error event, and never fabricates an AI response. |
...
| Cohere V2 streamed function call | Text, JSON argument deltas, tool-call ID/name, usage, tools, and `strict_tools` are translated into neutral turn records. |
