"""Report email delivery. Zero-dependency: stdlib smtplib + email.message over the Brevo relay
(mirrors the Hermes sender). No new package, no LiteLLM-style abstraction.
...
def render_report_email(report: dict) -> tuple[str, str, str]:
    ...
    name = escape(report["session"].get("display_name") or "Candidate")
...
def send_report_email(to: str, report: dict) -> None:
    """Blocking SMTP send — call via anyio.to_thread. Raises on failure; the router maps that to 502."""
    s = get_settings()
    subject, html, text = render_report_email(report)
    msg = EmailMessage()
    msg["Subject"] = subject
    msg["From"] = s.email_address
    msg["To"] = to
    msg.set_content(text)                    # text/plain fallback part
    msg.add_alternative(html, subtype="html")
    with smtplib.SMTP(s.email_smtp_host, s.email_smtp_port, timeout=15) as smtp:
        smtp.starttls()
        smtp.login(s.email_smtp_user or s.email_address, s.email_password)
