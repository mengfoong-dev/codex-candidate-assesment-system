...
func run(_tree: SceneTree) -> Array[String]:
    ...
    t.assert_equal(summary.final_hypothesis.label, "Independent watch-page lookups are waiting sequentially.", "final hypothesis label")
    ...
    t.assert_equal(summary.evidence_timeline[2].title, "Watch-Page Request Trace", "evidence title mapping")
    ...
    t.assert_equal(summary.final_submission.root_cause.label, "Sequential independent watch-page calls", "root cause label mapping")
